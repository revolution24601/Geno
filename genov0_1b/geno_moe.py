...
class Experts(nn.Module):
    ...
    def forward(self, x):
        hidden = torch.einsum("...nd,...dh->...nh", x.float(), self.w1.float())
        hidden = self.act(hidden)
        out = torch.einsum("...nh,...hd->...nd", hidden.float(), self.w2.float())
        return out
...
class MoE(nn.Module):
    ...
    def forward(self, inputs, **kwargs):
        ...
        #     dispatch_tensor[:, :, 1:] = 0
        # elif 2 <= epoch < 3:
        #     dispatch_tensor[:, :, 2:] = 0  # 选择前两个专家
        # elif 3 <= epoch < 4:
        #     dispatch_tensor[:, :, 3:] = 0  # 选择前三个专家
        # elif epoch >= 4:
        #     dispatch_tensor[:, :, 4:] = 0  # 选择前四个专家

        expert_inputs = torch.einsum(
            "bnd,bnec->ebcd", inputs.float(), dispatch_tensor.float()
        )

        # Now feed the expert inputs through the experts.
        orig_shape = expert_inputs.shape
        expert_inputs = expert_inputs.reshape(e, -1, d)
        expert_outputs = self.experts(expert_inputs.float())
